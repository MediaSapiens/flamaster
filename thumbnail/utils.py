...
def set_scale(image, geometry, options):
    ...
    if upscale:
        ...
        factor = crop and max(factors) or min(factors)
        if factor < 1:
...
def get_thumbnail(file_name, geometry_string, options_string):
    """
    ...
    """
    image_name = os.path.basename(file_name).rpartition('.')[0]
    options = configure_options(options_string)
    thumbnail_name = get_thumbnail_name(image_name, geometry_string, options)
    thumbnail = os.path.isfile(thumbnail_name) and thumbnail_name
    if not thumbnail:
        source_image = Image.open(file_name)
        thumbnail = create_thumbnail(source_image, geometry_string,
